...
/*   Updated: 2021/03/15 15:28:19 by dbliss           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void identify_from_reference( Base & p)
{
	try {
		A & childA = dynamic_cast<A&>(p);
		std::cout << "Identification from reference: type A" << std::endl;
	}
	...
	try {
		B & childB = dynamic_cast<B&>(p);
		std::cout << "Identification from reference: type B" << std::endl;
	}
	catch (std::bad_cast &bc)
	{}
	try {
		C & childC = dynamic_cast<C&>(p);
		std::cout << "Identification from reference: type C" << std::endl; 
	}
	catch (std::bad_cast &bc)
	{}
